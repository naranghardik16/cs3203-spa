...
std::shared_ptr<ClauseEvaluator> PatternClauseSyntax::CreateClauseEvaluator(Synonym s, Map &declaration_map, std::shared_ptr<PkbReadFacade> pkb) {
  std::shared_ptr<ClauseEvaluator> eval = std::make_shared<AssignPatternClauseEvaluator>(s, declaration_map, ClauseSyntax::GetSyntaxPair(), pkb);
  return eval;
}
