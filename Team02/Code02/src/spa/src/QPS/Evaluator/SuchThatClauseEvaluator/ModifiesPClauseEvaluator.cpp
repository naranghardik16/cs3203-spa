...
bool ModifiesPClauseEvaluator::EvaluateBooleanConstraint(std::shared_ptr<PkbReadFacade> pkb) {
  ...
  // So first arg must be ident since first arg can either be syn (non-boolean constraint) or int (falls under
  // ModifiesS)
  ...
  if (is_second_arg_a_wildcard) {
    ...
    return pkb->HasModifiesProcedureRelationship(QueryUtil::GetIdent(first_arg_),
