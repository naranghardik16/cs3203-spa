...
TEST_CASE("Testcases for Next Store") {
  SECTION("Empty Next Store") {
    ...
    REQUIRE(next_store->hasAnyNextStarRelation() == false);
    REQUIRE(next_store->hasNextStarRelation("2", "3") == false);
    ...
    REQUIRE(next_store->retrieveAllNextStarPairs() == std::unordered_set<std::pair<PkbTypes::STATEMENT_NUMBER,
                                                                                         PkbTypes::STATEMENT_NUMBER>,
                                                                               PairHasherUtil::hash_pair>({}));
  }
  ...
  SECTION("Independent Next without any Transitive Relationship") {
    auto next_store = new NextStore();
    next_store->addNextRelation("1", "2");
    next_store->addNextRelation("4", "8");
    ...
    REQUIRE(next_store->hasAnyNextStarRelation() == true);
  }
}