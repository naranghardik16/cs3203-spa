...
bool NextClauseEvaluator::EvaluateBooleanConstraint(std::shared_ptr<PkbReadFacade> pkb) {
  ...
  if (is_first_arg_a_wildcard && is_second_arg_a_wildcard) {
    ...
    return pkb->HasNextBy(second_arg_);
  } else if (is_second_arg_a_wildcard) {
    //Example query: Next(1, _)

    return pkb->HasNext(first_arg_);
  } else {
    //Example query: Next(1, 6)

    return pkb->IsNext(first_arg_, second_arg_);
  }
}

std::shared_ptr<Result> NextClauseEvaluator::EvaluateClause(std::shared_ptr<PkbReadFacade> pkb) {
  ...
  if (is_first_arg_synonym && is_second_arg_synonym) {
    ...
    single_constraint = pkb->GetNextFirst(arg_1_type);
  } else if (is_first_arg_synonym) {
    //Example query: Next (s, 1)

    single_constraint = pkb->GetNextBy(second_arg_, arg_1_type);
  } else if (is_first_arg_a_wildcard) {
    //Example query: Next (_, s)

    single_constraint = pkb->GetNextSecond(arg_2_type);
  } else {
    //Example query: Next (1, s)

    single_constraint = pkb->GetNext(first_arg_, arg_2_type);
  }
