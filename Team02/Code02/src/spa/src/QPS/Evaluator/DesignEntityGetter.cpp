...
std::shared_ptr<Result> DesignEntityGetter::EvaluateBasicSelect(const Synonym& synonym,
  ...
  ResultHeader header;
  header[token_lst[0]] = static_cast<int>(header.size());
  if (token_lst.size() == 2) {
    header[synonym] = static_cast<int>(header.size());
  }
