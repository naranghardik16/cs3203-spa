...
std::vector<std::shared_ptr<ClauseGroup>> Optimizer::GetClauseGroups(const ClauseSyntaxPtrList& clause_list) {
  ...
  for (const auto &kClause : clause_list) {
    ...
    if (syn.empty()) {
      if (groups.find(pql_constants::NoSynClauseGroupKey) == groups.end()) {
        groups[pql_constants::NoSynClauseGroupKey] = std::make_shared<ClauseGroup>();
      }
      groups[pql_constants::NoSynClauseGroupKey]->AddClause(kClause);
    } else {
